...
>> 3. `<loop>` : 오디오나 비디오를 자동으로 실행한다 
>> 4. `<muted>` : 오디오나 비디오 소리를 제거한다
>> 5. `<preload>` : auto 기본값, metadata, none
>> 6. `<width>,<height>` : 너비와 높이를 지정한다
>> 7. `<poster="파일 이름">` : video에서 사용하는 태그로 비디오가 재생되기 전까지 화면에 표시될 포스터의 이미지를 지정한다
